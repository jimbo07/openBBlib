...
from ooutdmaya.rigging.core.util.IO import meshData, data
...
class DeformerData( data.Data ):
    ...
    # Example import export convenience functions
    
    from ooutdmaya.rigging.core.util.IO import deformerData
    
    filePath = '/tmp/tmp.wgt'
    deformer = 'pSphere1_vts'
    geometry = 'pSphere1'
    
    deformerData.exportToFile(deformer, filePath)
    deformerData.importFromFile(deformer, geometry, filePath)
    '''
    ...
    def buildData(self,deformer):
        ...
        affectedGeo = easy.getAffectedGeometry(deformer,returnShapes=False)
        self._data['affectedGeometry'] = [str(i) for i in easy.dict_orderedKeyListFromValues(affectedGeo)]
        ...
        for geo in self._data['affectedGeometry']:
            ...
            self._data[geo]['membership'] = easy.getDeformerSetMemberIndices(deformer,geo)
            self._data[geo]['weights'] = easy.getWeights(deformer, geo)
    ...
    def remapGeometry(self, oldGeometry=None, newGeometry=None):
        '''
        ...
        # - Checks -
        # ==========
        oldGeometry = oldGeometry or self._data['affectedGeometry'][0]
        if not newGeometry:
            return None
    ...
    def setDeformerMembership(self,geoList=[]):
        ...
        deformerSet = easy.getDeformerSet(deformer)
        
        for geo in geoList:
            
            # Get Current and Stored Member Data
            setMembers = self._data[geo]['membership']
            currMembers = easy.getDeformerSetMemberIndices(deformer,geo)
            ...
            if removeMembers:
                ...
                # Lattice
                if self._data[geo]['geometryType'] == 'lattice':
                    cmds.sets([geo+'.pt['+str(i[0])+']['+str(i[1])+']['+str(i[2])+']' for i in removeMembers],rm=deformerSet)
    ...
    def loadWeights(self,geoList=[]):
        ...
        for geo in geoList:
            ...
            easy.setWeights(deformer, wt, geo)

    
    def rebuildMeshFromData(self, mesh):
        ''' Rebuild the specified mesh from the stored deformer data
        :param mesh str: Mesh to rebuild geometry from data for. 
        '''
        # Checks Mesh Data
        if not self._data.has_key(mesh):
            raise Exception('No data stored for mesh {0}!'.format(mesh))
    ...
    def rebuildMeshData(self, mesh, sourceMesh=''):
        ...
        :param mesh str: Mesh to rebuild geometry from data for. 
        :param sourceMesh str: The source mesh to rebuild the data from. 
        '''
        # Check Source Mesh
        if not sourceMesh: sourceMesh = mesh
        if not cmds.objExists(sourceMesh):
            raise Exception('Source mesh {0} does not exist! Unable to rebuild mesh data...'.format(sourceMesh))
    ...
    def rebuildWorldSpaceData(self,sourceGeo, targetGeo='', method='closestPoint'):
        ...
        :param sourceGeo str: Geometry to rebuild world space deformer data from.
        ...
        # Display Progress
        progressBar.init(status=('Rebuilding world space deformer data...'), maxValue=100)
        ...
        for i in range(numTargetVerts):
            ...
            for n in range(3):
                ...
                triId = om.MScriptUtil().getIntArrayItem(idPtr,n)
    ...
    def getDeformerAttrConnections(self):
        ...
        for attr in self._data['attrConnectionList']:
            attrConn = cmds.listConnections(deformer+'.'+attr,s=True,d=False,p=True,sh=True,skipConversionNodes=True)
...
def exportToFile(deformer, filePath):
    """
    # Example
    from ooutdmaya.rigging.core.util.IO import deformerData
    ...
    deformerData.exportToFile(deformer, filePath)
    """
    dd = DeformerData(deformer=deformer)
    
    # Export
    dd.save(filePath=filePath, force=True)
    
    return dd


def importFromFile(deformer, geometry, filePath, setMembership=False):
    """
    # Example
    from ooutdmaya.rigging.core.util.IO import deformerData
    ...
    # Import
    dd = dd.load(filePath=filePath)
    
    affctdGeo = dd._data['affectedGeometry'][0]
    if dd._data[affctdGeo].has_key('mesh'):
        dd.remapGeometry(newGeometry=geometry)
    
    # Rebuild Deformer
    try: dd.rebuild(setMembership=False)
    except: cmds.warning("{0} - failed to rebuild, no worldspace mesh has been stored and point" +
                         " index is a mismatch!".format(filePath))
